...
const compactSheetInventoryBag = (rawBag: unknown[]) => {
  ...
  for (const rawItem of rawBag) {
    ...
    const key = getSheetInventoryStackKey(item);
...
const mergeSheetInventoryItemIntoBag = (rawBag: unknown[], rawItem: Record<string, any>, rawQty = 1) => {
  ...
  const item = { ...(rawItem || {}), qty: Math.max(1, Math.floor(Number(rawQty) || 1)) };
  const key = getSheetInventoryStackKey(item);
  const existingIndex = bag.findIndex((entry) => getSheetInventoryStackKey(entry) === key);
...
export default function CharacterSheetScreen() {
  ...
  const applyLanInventoryPatchToCharacter = useCallback(async (patch: LanSessionEvent['inventoryPatch'], event?: LanSessionEvent) => {
    ...
    // Grants do mestre precisam ser stackáveis e resilientes a snapshot stale.
    // O equipment completo continua sendo o estado final autoritativo, mas quando
    // ele vier igual/atrasado, o itemDelta garante que Faca 1x -> 2x -> 3x aplique
    // imediatamente no inventário do jogador.
    if (String(patch.action || '') === 'grant' && itemDelta?.mode === 'add' && itemDelta.item && typeof itemDelta.item === 'object') {
      const deltaItem = itemDelta.item as Record<string, any>;
      const deltaQty = Math.max(1, Math.floor(Number(itemDelta.qty) || 1));
      const deltaKey = getSheetInventoryStackKey({ ...deltaItem, qty: deltaQty });
      const currentEntry = currentEquipment.bag.find((entry: any) => getSheetInventoryStackKey(entry) === deltaKey);
      const incomingEntry = incomingEquipment.bag.find((entry: any) => getSheetInventoryStackKey(entry) === deltaKey);
      const currentQty = Math.max(0, Number((currentEntry as any)?.qty || 0));
      const incomingQty = Math.max(0, Number((incomingEntry as any)?.qty || 0));
    ...
    const nextStatsPatch = event?.statsPatch && typeof event.statsPatch === 'object'
    ...
    setCharacter((prev: any) => {
      if (!prev) return prev;
      const merged = { ...prev, equipment: nextEquipment, ...(nextStatsPatch ? { stats: nextStatsPatch } : {}) };
      characterRef.current = merged;
      return merged;
    });
    ...
    if (nextStatsPatch) {
      await db.runAsync(
        ...
        [JSON.stringify(nextEquipment), JSON.stringify(nextStatsPatch), Number(currentCharacter.id)]
      );
    } else {
      await db.runAsync(
  ...
  const sendResourceRequest = async (request: LanResourceRequest) => {
    ...
    try {
      ...
      // v41: pedido de recurso não deve disparar re-join/snapshot.
      // O host cria/reativa o jogador pelo proprio evento quando necessário.
    ...
    try {
      ...
      const event: LanSessionEvent = {
        ...
        inventoryPatch: {
          targetKey: selfKey,
          equipment,
          baseEquipment: baseEquipment || characterRef.current?.equipment || character.equipment,
          reason,
          action: 'self_update',
        },
        statsPatch,
  ...
  const sendCoinSelfPatchRequest = async (
    ...
    const currentCharacter = characterRef.current || character;
    if (!lanInfo?.joinUrl || !currentCharacter) return false;
    const selfKey = getSelfLanKey(lanInfo.sessionId);
    ...
    coinPatchDebounceRef.current = {
      ...
      timer: setTimeout(() => {
        ...
        void (async () => {
          try {
            await sendLanSessionEvent(lanInfo.joinUrl, queuedEvent);
            await rememberLanSessionEvent(db, queuedEvent).catch(() => false);
  ...
  const applyStructuredItemEffects = async (bagIndex: number, rawItem: any, qty: number, effects: any[], chosenAttr?: string) => {
    ...
    await updateDB(dbUpdates, { reason: isLanPlayerRuntime ? 'lan_item_consume_runtime_first' : 'offline_item_consume' });
    setCharacter((prev: any) => prev ? ({
      ...prev,
      ...dbUpdates,
    ...
    if (isLanPlayerRuntime && lanInfo?.sessionId) {
      ...
      for (const effectToNotify of lanEffectsToNotify.filter((effect: any) => !effect?.isPermanent && effect?.unit !== 'permanent')) {
        void notifySelfLanSpellEvent({
          spellName: effectToNotify.name || item.name,
          mode: 'effect',
          target: effectToNotify.target,
          value: effectToNotify.value,
          durationText: effectToNotify.durationText,
          durationRemaining: effectToNotify.remaining,
          durationUnit: effectToNotify.unit,
