...
import { enqueueLanEntityMutation, getLanPlayerQueueKey, getLanSessionQueueKey } from '@/services/lan/lanEntityQueue';
import {
...
function mergeMasterTimelineEvent(current: LanSessionEvent[], event: LanSessionEvent | null | undefined, limit = 40) {
...
function addItemToHostEquipment(equipment: unknown, item?: Partial<LanTradeItem> | null, fallbackQty = 1) {
  ...
  const index = bag.findIndex((entry: any) => normalizeHostItemName(entry?.name) === normalizeHostItemName(name));
  if (index >= 0) {
...
function removeItemFromHostEquipment(equipment: unknown, item?: Partial<LanTradeItem> | null, fallbackQty = 1) {
  ...
  const bag = Array.isArray(nextEquipment.bag) ? [...nextEquipment.bag] : [];
  const index = bag.findIndex((entry: any) => normalizeHostItemName(entry?.name) === normalizeHostItemName(name));
  if (index < 0) return { equipment: nextEquipment, removed: false };
  const current = bag[index];
  const nextQty = Math.max(0, Math.floor(Number(current?.qty || 0)) - qty);
  if (nextQty <= 0) bag.splice(index, 1);
  else bag[index] = { ...current, qty: nextQty };
  return { equipment: { ...nextEquipment, bag }, removed: true };
}
...
export default function LanSessionScreen() {
  ...
  const sendPublicPlayerStatus = useCallback(async (
    ...
    const hpCurrent = Math.max(0, Math.floor(Number(numberPatch?.hpCurrent ?? player.hpCurrent) || 0));
    const hpMax = Math.max(0, Math.floor(Number(numberPatch?.hpMax ?? player.hpMax) || 0));
    const tempHp = Math.max(0, Math.floor(Number(numberPatch?.tempHp ?? player.tempHp) || 0));
    const seq = Date.now();
    const event: LanSessionEvent = {
      ...
      publicState: {
        ...
        tempHp,
        level: player.level,
        effects: summarizePlayerPublicEffects(player.effects),
      },
      message: `${player.characterName} atualizou status publico (${reason}).`,
  ...
  const applyAndSendEffectResult = useCallback(async (
    ...
    if (runtimeResult) {
      ...
      const publicPlayer = runtimeResult.state.players.find((entry) => entry.remoteKey === result.targetKey);
    ...
    return fallback;
  }, [sendLatestLiveEvent, sendLiveEventToClients, sendPublicPlayerStatus]);
  ...
  const sendOfficialInventoryPatch = useCallback(async (
    ...
    const entityRevision = Math.max(0, Number(player.revisionSeq || 0)) + 1;
    ...
    const sent = await sendLanSessionEvent(joinUrl, event).catch((error) => {
  ...
  const handleHostCoinSelfPatchEvent = useCallback(async (
    ...
    const committedEvent: LanSessionEvent = {
      ...
      fromKey: 'master', fromName: 'Mestre', toKey: patchedPlayer.remoteKey, toName: patchedPlayer.characterName,
      entityType: 'player', entityId: patchedPlayer.remoteKey, entityRevision: runtimePatch.revision,
  ...
  const handleHostInventoryPatchEvent = useCallback(async (
    ...
    void enqueueLanEntityMutation([getLanPlayerQueueKey(activeSessionId, runtimePlayer.remoteKey)], async () => {
      ...
      if (applied && event.statsPatch && typeof event.statsPatch === 'object') {
        const nextStats = sanitizePlayerOwnedStatsPatch(runtimePlayer.stats, event.statsPatch as Record<string, unknown>);
    ...
    return true;
  }, [activeSessionId, applyHostInventoryRuntimePatch, db, rememberSentEventInTimeline, scheduleSilentPayloadRefresh, sendOfficialInventoryPatch]);
  ...
  const handleHostSendItemRequestFast = useCallback(async (event: LanSessionEvent) => {
    ...
    const sourceAfter = event.sendItemRequest.sourceEquipmentAfter
      ? cloneEquipmentForHost(event.sendItemRequest.sourceEquipmentAfter)
      : removeItemFromHostEquipment(sourcePlayer.equipment, item, qty).equipment;
    ...
    if (sourcePatched?.remoteKey) void sendOfficialInventoryPatch(activeSessionId, sourcePatched, reason, 'transfer_out', event.clientMsgId || event.id);
    if (targetPatched?.remoteKey) void sendOfficialInventoryPatch(activeSessionId, targetPatched, reason, 'transfer_in', event.clientMsgId || event.id);
  ...
  const handleHostTradeAcceptFast = useCallback(async (event: LanSessionEvent) => {
    ...
    const eventId = String(event.id || '');
    if (eventId) seenNativeHostEventIdsRef.current.add(eventId);
    const actionId = `trade_accept:${event.tradeId || event.tradeAccept?.tradeId || event.clientMsgId || event.id}`;
    if (processedHostActionIdsRef.current.has(actionId)) {
      debugLanFlow('MASTER_TRADE_FAST_DUPLICATE_IGNORED', { sessionId: activeSessionId, eventId: event.id, actionId });
      return true;
    }
    processedHostActionIdsRef.current.add(actionId);

    const state = sessionStateRef.current;
    const fromKey = String(event.tradeAccept?.fromKey || event.fromKey || '');
    const toKey = String(event.tradeAccept?.toKey || (event.toKey !== 'master' ? event.toKey : '') || '');
    const fromPlayer = state?.players.find((entry) => entry.remoteKey === fromKey);
    const toPlayer = state?.players.find((entry) => entry.remoteKey === toKey);
    ...
    if (!state || !fromPlayer?.remoteKey || !toPlayer?.remoteKey || !offered?.name) {
      debugLanFlow('MASTER_TRADE_FAST_FALLBACK_SQLITE', { sessionId: activeSessionId, eventId: event.id, fromKey, toKey, hasOffered: Boolean(offered?.name) });
      return false;
    }

    const fromRemoved = removeItemFromHostEquipment(fromPlayer.equipment, offered, offered.qty || 1);
    let fromAfter = requested?.name ? addItemToHostEquipment(fromRemoved.equipment, requested, requested.qty || 1) : fromRemoved.equipment;
    let toAfter = addItemToHostEquipment(toPlayer.equipment, offered, offered.qty || 1);
    if (requested?.name) {
      toAfter = removeItemFromHostEquipment(toAfter, requested, requested.qty || 1).equipment;
    }
    const fromPatched = applyHostInventoryRuntimePatch(fromKey, fromAfter);
    const toPatched = applyHostInventoryRuntimePatch(toKey, toAfter);
    const reason = event.message || `${fromPlayer.characterName} concluiu uma troca com ${toPlayer.characterName}.`;

    rememberSentEventInTimeline({ ...event, message: reason });
    if (fromPatched?.remoteKey) void sendOfficialInventoryPatch(activeSessionId, fromPatched, reason, 'replace', event.clientMsgId || event.id);
    if (toPatched?.remoteKey) void sendOfficialInventoryPatch(activeSessionId, toPatched, reason, 'replace', event.clientMsgId || event.id);
    void sendTradeResult(activeSessionId, fromPlayer.remoteKey, fromPlayer.characterName, event.tradeId, true, reason);
    void sendTradeResult(activeSessionId, toPlayer.remoteKey, toPlayer.characterName, event.tradeId, true, reason);
    void enqueueLanEntityMutation([getLanPlayerQueueKey(activeSessionId, fromPlayer.remoteKey), getLanPlayerQueueKey(activeSessionId, toPlayer.remoteKey)], async () => {
      await rememberLanSessionEvent(db, event).catch(() => false);
    ...
    debugLanFlow('MASTER_TRADE_FAST_RUNTIME_COMMITTED_DIRECT', { sessionId: activeSessionId, eventId: event.id, tradeId: event.tradeId, fromKey, toKey });
    return true;
  ...
  const reloadSessionState = useCallback(async (sessionId: string, syncPayload = false) => {
    ...
    if (!syncPayload && now - reloadSessionStateLastRunAtRef.current < 2500) {
...
useEffect(() => {
  ...
  const handleAdvanceTime = async (unit: LanAdvanceUnit) => {
    ...
    for (const player of runtimeResult.state.players) {
      ...
      if (removed.length || updated.length) {
        ...
        const effectPatchEvent: LanSessionEvent = {
          ...
          originClientId: 'master',
          effectPatch: patch,
          message: `${player.characterName}: efeitos atualizados pela passagem de tempo.`,
          createdAt: new Date().toISOString(),
        };
    ...
    void enqueueLanEntityMutation([getLanSessionQueueKey(sessionId)], async () => {
      ...
      await advanceLanSessionTime(db, sessionId, unit, { syncPayload: false });
  ...
  const persistHostNumberPatch = useCallback(async (playerId: number, patch: NumberPatch, message: string) => {
    ...
    void sendLanSessionEvent(joinUrl, liveEvent)
      .then(() => {
        ...
        void sendPublicPlayerStatus(sessionId, targetPlayer, 'number_patch', authoritativePatch);
      })
      .catch((error) => {
  ...
  }, [db, joinUrl, payload?.session?.id, scheduleSilentPayloadRefresh, sendPublicPlayerStatus]);
  ...
  const persistHostEffectPatch = useCallback((
    ...
    // O histórico da partida precisa aparecer junto com o efeito, não depois do ACK.
    ...
    void sendLanSessionEvent(joinUrl, liveEvent)
      .then(() => {
        ...
        void sendPublicPlayerStatus(sessionId, runtimeResult.player, 'effect_patch');
      })
      .catch((error) => {
  ...
  const handleUpdatePlayerDelta = async (
    ...
    if (!payload || endingSessionRef.current) return;
    const sessionId = payload.session.id;
    await enqueueLanEntityMutation([getLanPlayerQueueKey(sessionId, player.id)], async () => {
  ...
  const handleReviewResourceRequest = async (event: LanSessionEvent, accepted: boolean) => enqueueMasterMutation(async () => {
    ...
    if (request.kind === 'inventory' && request.item) {
      ...
      const runtimePlayer = applyHostInventoryRuntimePatch(targetPlayer.remoteKey, nextEquipment);
      if (runtimePlayer?.remoteKey) {
        void sendOfficialInventoryPatch(sessionId, runtimePlayer, reviewEvent.message, 'grant', resourceRequestId, {
          itemDelta: {
            mode: 'add',
        ...
        void enqueueLanEntityMutation([getLanPlayerQueueKey(sessionId, runtimePlayer.remoteKey)], async () => {
          await rememberLanSessionEvent(db, event).catch(() => false);
  ...
  const handleApplyQuickEdit = async () => runMasterAction('quick_edit', async () => {
    ...
    if (type === 'XP') {
       // v35: fecha o modal antes da persistencia/broadcast para nunca ficar
       // preso visualmente em "SALVANDO..." se o socket/SQLite estiverem lentos.
       setQuickEdit(null);
       await handleUpdatePlayer(player, 'xp', Math.max(0, player.xp + val));
       return;
    } else if (type === 'COIN') {
